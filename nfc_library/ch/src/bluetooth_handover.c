...
#include <stdint.h>
#include <string.h>
#include "../../ndef/inc/ndef_message.h"
#include "../inc/bluetooth_handover.h"

#define BLUETOOTH_LE_OOB_RECORD_TYPE_LEN        (32)
#define BLUETOOTH_EP_OOB_RECORD_TYPE_LEN        (32)

/// Bluetooth LE out-of-band record type.
const uint8_t bluetooth_le_oob_record_type[] = "application/vnd.bluetooth.le.oob";
/// Bluetooth BR/EDR out-of-band record type.
const uint8_t bluetooth_ep_oob_record_type[] = "application/vnd.bluetooth.ep.oob";
 ...
 *****************************************************************************/
void ch_bluetooth_le_carrier_configuration_record_encode (ndef_record_t* record, bt_carrier_config_t* config) {
  ...
  record->payload[index++] = BLUETOOTH_AD_TYPE_FIELD_LEN + BLUETOOTH_LE_MAC_ADDR_FIELD_LEN;
  /* Set LE Bluetooth Devicess Address AD type and increment index by 1.*/
  record->payload[index++] = BLUETOOTH_AD_TYPE_LE_DEVICE_ADDR;
  /* Set MAC address. */
  memcpy(&record->payload[index], config->addr.value, BLUETOOTH_LE_MAC_ADDR_FIELD_LEN);
  /* Increment index by length of value.  */
  index += BLUETOOTH_LE_MAC_ADDR_FIELD_LEN;

  /* Set LE Role length and increment index by 1. */
  record->payload[index++] = BLUETOOTH_AD_TYPE_FIELD_LEN + BLUETOOTH_LE_ROLE_FIELD_LEN;
  ...
  if (config->security_manager_tk_value.is_set) {
    ...
    record->payload[index++] = BLUETOOTH_AD_TYPE_FIELD_LEN + BLUETOOTH_SM_TK_VALUE_FIELD_LEN;
    /* Set Security Manager TK value AD type. */
    record->payload[index++] = BLUETOOTH_AD_TYPE_SM_TK_VALUE;
    /* Set Security Manager TK value. */
    memcpy(&record->payload[index], config->security_manager_tk_value.value, BLUETOOTH_SM_TK_VALUE_FIELD_LEN);
    /* Increment index by BLUETOOTH_SM_TK_VALUE_field_LEN. */
    index+= BLUETOOTH_SM_TK_VALUE_FIELD_LEN;
  }

  /* Check if Secure Connections Confirmation value is included. If so, set its values. */
  if (config->secure_connections_confirm_value.is_set) {
    /* Set Secure Connections Confirmation value field length and increment index by 1. */
    record->payload[index++] = BLUETOOTH_AD_TYPE_FIELD_LEN + BLUETOOTH_SC_CONFIRMATION_VALUE_FIELD_LEN;
    ...
    memcpy(&record->payload[index], config->secure_connections_confirm_value.value, BLUETOOTH_SC_CONFIRMATION_VALUE_FIELD_LEN);
    /* Increment index by BLUETOOTH_SC_CONFIRMATION_VALUE_FIELD_LEN.*/
    index+= BLUETOOTH_SC_CONFIRMATION_VALUE_FIELD_LEN;
  }

  /* Check if Secure Connections Random value is included. If so, set its values. */
  if (config->secure_connectinos_random_value.is_set) {
    /* Set Secure Connections Random value field length and increment index by 1. */
    record->payload[index++] = BLUETOOTH_AD_TYPE_FIELD_LEN + BLUETOOTH_SC_RANDOM_VALUE_FIELD_LEN;
    ...
    memcpy(&record->payload[index], config->secure_connectinos_random_value.value, BLUETOOTH_SC_RANDOM_VALUE_FIELD_LEN);
  ...
  if (config->appearance.is_set) {
    ...
    record->payload[index++] = BLUETOOTH_AD_TYPE_FIELD_LEN + BLUETOOTH_APPEARANCE_FIELD_LEN;
    /* Set Appearance AD type. */
    record->payload[index++] = BLUETOOTH_AD_TYPE_APPEARANCE;
    /* Set Appearance. */
    memcpy(&record->payload[index], config->appearance.value, BLUETOOTH_APPEARANCE_FIELD_LEN);
  ...
  if (config->local_name.is_set) {
    ...
    record->payload[index++] = BLUETOOTH_AD_TYPE_FIELD_LEN + config->local_name.length;
    /* Set Local Name AD type. */
    record->payload[index++] = config->local_name.type;
    /* Set Local Name. */
    memcpy(&record->payload[index], config->local_name.value, config->local_name.length);
  ...
  if (config->flags.is_set) {
    ...
    record->payload[index++] = BLUETOOTH_AD_TYPE_FIELD_LEN + BLUETOOTH_FLAGS_FIELD_LEN;
    /* Set Flags AD type. */
