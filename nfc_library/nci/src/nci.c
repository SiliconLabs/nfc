...
#include <stdint.h>
#include "../inc/nci.h"
#include "../inc/nci_tml.h"

#define NCI_PACKET_HEADER_LEN                               (3)
#define NCI_PACKET_PAYLOAD_MAX_LEN                          (255)
#define NCI_PACKET_MAX_LEN                                  (NCI_PACKET_HEADER_LEN + NCI_PACKET_PAYLOAD_MAX_LEN)
...
extern nci_err_t nci_incoming_proprietary_packet_ntf_process (uint8_t* packet_buff);
...
nci_err_t nci_data_packet_send (nci_data_packet_t* packet) {
  ...
  packet_buff[NCI_PACKET_MT_INDEX]  &= ~NCI_PACKET_MT_M;
  packet_buff[NCI_PACKET_MT_INDEX]  += (nci_packet_data << NCI_PACKET_MT_SHIFT);
  packet_buff[NCI_PACKET_PBF_INDEX] &= ~NCI_PACKET_PBF_M;
  packet_buff[NCI_PACKET_PBF_INDEX] += (packet->pbf << NCI_PACKET_PBF_SHIFT);
  packet_buff[NCI_PACKET_CONN_ID_INDEX] &= ~NCI_PACKET_CONN_ID_M;
  packet_buff[NCI_PACKET_CONN_ID_INDEX] += (packet->conn_id << NCI_PACKET_CONN_ID_SHIFT);
...
static void nci_core_reset_rsp_process (void) {
  ...
  nci_evt.data.payload.nci_data.core_reset_rsp.status = packet_buff[index++];
  nci_evt.data.payload.nci_data.core_reset_rsp.nci_version = packet_buff[index++];
  nci_evt.data.payload.nci_data.core_reset_rsp.config_status = packet_buff[index];
}
...
static void nci_core_reset_ntf_process (void) {
  ...
  nci_evt.data.payload.nci_data.core_reset_ntf.reason_code = packet_buff[index++];
  nci_evt.data.payload.nci_data.core_reset_ntf.config_status = packet_buff[index];
}
...
static void nci_core_init_rsp_process (void) {
  ...
  nci_evt.data.payload.nci_data.core_init_rsp.nfcc_feature = &packet_buff[index];
  index += 4;
  nci_evt.data.payload.nci_data.core_init_rsp.num_rf_interface = packet_buff[index++];
  nci_evt.data.payload.nci_data.core_init_rsp.rf_interfaces = &packet_buff[index];
  index += nci_evt.data.payload.nci_data.core_init_rsp.num_rf_interface;
  nci_evt.data.payload.nci_data.core_init_rsp.max_logical_conn = packet_buff[index++];
  nci_evt.data.payload.nci_data.core_init_rsp.max_routing_tab_size = &packet_buff[index];
  index += 2;
  nci_evt.data.payload.nci_data.core_init_rsp.max_ctrl_pkt_payload_size = packet_buff[index++];
  nci_evt.data.payload.nci_data.core_init_rsp.max_size_large_param = &packet_buff[index];
  index += 2;
  nci_evt.data.payload.nci_data.core_init_rsp.manu_id = packet_buff[index++];
  nci_evt.data.payload.nci_data.core_init_rsp.manu_spec_info = &packet_buff[index];
}
 ...
 *  Any error code.
 *****************************************************************************/
nci_err_t nci_rf_set_listen_mode_routing (nci_rf_set_listen_mode_routing_cmd_t* cmd) {
  ...
  for (int i = 0; i < cmd->num_of_routing_entries; i++) {
      ...
      memcpy(&payload[payload_index], &cmd->routing_entries[i].value, cmd->routing_entries[i].length);
...
static void nci_rf_discover_ntf_process (void) {
  ...
  nci_evt.data.payload.nci_data.rf_discover_ntf.rf_disc_id = packet_buff[index++];
  nci_evt.data.payload.nci_data.rf_discover_ntf.rf_protocol = packet_buff[index++];
  nci_evt.data.payload.nci_data.rf_discover_ntf.rf_tech_and_mode = packet_buff[index++];
  nci_evt.data.payload.nci_data.rf_discover_ntf.rf_tech_param_len = packet_buff[index++];

  switch (nci_evt.data.payload.nci_data.rf_discover_ntf.rf_tech_and_mode) {
    case nci_nfc_a_passive_poll_mode:
      nci_evt.data.payload.nci_data.rf_discover_ntf.rf_tech_param->nfc_a_poll_params.sens_res = &packet_buff[index];
      index += 2;
      nci_evt.data.payload.nci_data.rf_discover_ntf.rf_tech_param->nfc_a_poll_params.nfcid1_len = packet_buff[index++];
      nci_evt.data.payload.nci_data.rf_discover_ntf.rf_tech_param->nfc_a_poll_params.nfcid1 = &packet_buff[index];
      index += nci_evt.data.payload.nci_data.rf_discover_ntf.rf_tech_param->nfc_a_poll_params.nfcid1_len;
      nci_evt.data.payload.nci_data.rf_discover_ntf.rf_tech_param->nfc_a_poll_params.sel_res_len = packet_buff[index++];
      if(nci_evt.data.payload.nci_data.rf_discover_ntf.rf_tech_param->nfc_a_poll_params.sel_res_len == 1) {
          nci_evt.data.payload.nci_data.rf_discover_ntf.rf_tech_param->nfc_a_poll_params.sel_res = packet_buff[index++];
      }
      break;
...
static void nci_rf_intf_activated_ntf_process (void) {
  ...
  nci_evt.data.payload.nci_data.rf_intf_activated_ntf.rf_disc_id = packet_buff[index++];
  nci_evt.data.payload.nci_data.rf_intf_activated_ntf.rf_interface = packet_buff[index++];
  nci_evt.data.payload.nci_data.rf_intf_activated_ntf.rf_protocol = packet_buff[index++];
  nci_evt.data.payload.nci_data.rf_intf_activated_ntf.act_rf_tech_and_mode = packet_buff[index++];
  nci_evt.data.payload.nci_data.rf_intf_activated_ntf.max_data_packet_payload_size = packet_buff[index++];
  nci_evt.data.payload.nci_data.rf_intf_activated_ntf.init_num_of_credits = packet_buff[index++];
  nci_evt.data.payload.nci_data.rf_intf_activated_ntf.len_rf_tech_params = packet_buff[index++];
...
nci_err_t nci_incoming_packet_process (void) {
  ...
  /* Check Message Type */
  switch ( (packet_buff[NCI_PACKET_MT_INDEX] & NCI_PACKET_MT_M) >> NCI_PACKET_MT_SHIFT ) {
    /* Data packet. */
    case nci_packet_data: {
      /* Set event header to data packet */
      nci_evt.header = nci_evt_data_packet_rec;
      /* Set PBF bit */
      nci_evt.data.is_terminator_packet = !(packet_buff[NCI_PACKET_PBF_INDEX] & NCI_PACKET_PBF_M) ? true : false;
      /* Set data length */
      nci_evt.data.len = packet_buff[NCI_PACKET_PAYLOAD_LEN_INDEX];
      /* Set connection id */
      nci_evt.data.payload.nci_data.rec_data_packet.conn_id = ((packet_buff[NCI_PACKET_CONN_ID_INDEX] & NCI_PACKET_CONN_ID_M) >> NCI_PACKET_CONN_ID_SHIFT);
      /* Assign payload pointer */
      nci_evt.data.payload.nci_data.rec_data_packet.payload = &packet_buff[NCI_PACKET_PAYLOAD_START_INDEX];
      break;
    }
    /* Control Packet - Response Message */
    case nci_packet_control_rsp: {
      /* Check GID */
      switch ( (packet_buff[NCI_PACKET_GID_INDEX] & NCI_PACKET_GID_M) >> NCI_PACKET_GID_SHIFT ) {
        /* NCI Core responses */
        case nci_gid_nci_core: {
          /* Check OID */
          switch ( (packet_buff[NCI_PACKET_OID_INDEX] & NCI_PACKET_OID_M) >> NCI_PACKET_OID_SHIFT ) {
